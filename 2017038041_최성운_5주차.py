...
def sort_data(len):
    for i in range(len):
        for j in range(3, 6):
            str(all_list[i][j])
    print("학번으로 정렬")
    num_list = sorted(all_list[:len], key=lambda number: number[1])
    print(num_list)
    print("학과로 정렬")
    name_list = sorted(all_list[:len], key=lambda number: number[0])
    print(name_list)
